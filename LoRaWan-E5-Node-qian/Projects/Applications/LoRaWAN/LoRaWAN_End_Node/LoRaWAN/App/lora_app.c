...
static void SendTxData(void)
{
   ...
   // HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_SET);
    ...
    HAL_Delay(500);
    HAL_UART_Receive(&huart1, rxData, sizeof(rxData), HAL_MAX_DELAY);  // Receive data
   ...
   // HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET);
    ...
    // Search for the labels and extract the corresponding values
    char *label = strtok(&rxData[9], "!");
    char *value;
    while (label != NULL) {
        // Check if the label starts with "P:"
        if (strncmp(label, "P:", 2) == 0) {
            value = label + 2; // Move the pointer past "P:"
            P = atoi(value)/1000;
        }
        // Repeat the same process for other labels (G:, H:, T:, C:)
        else if (strncmp(label, "G:", 2) == 0) {
            value = label + 2;
            G = atoi(value)/1000;
        } else if (strncmp(label, "H:", 2) == 0) {
            value = label + 2;
            H = atoi(value);
        } else if (strncmp(label, "T:", 2) == 0) {
            value = label + 2;
            T = atoi(value);
        } else if (strncmp(label, "C:", 2) == 0) {
            value = label + 2;
            C = atoi(value);
        }
	...
	int16_t gaz = (uint16_t) G;
	int16_t temperature = (uint16_t) T;
	int16_t humidity = (uint16_t) H;
	int16_t camera = (uint16_t) C;
	int16_t pressure = (uint16_t) P;
  ...
  AppData.Buffer[i++] = (uint16_t) gaz;
  AppData.Buffer[i++] = (uint16_t) temperature;
  AppData.Buffer[i++] =(uint16_t)  camera;
  AppData.Buffer[i++] =(uint16_t)  pressure;
